...
namespace SiliconValley.InformationSystem.Web.Areas.Dormitory.Controllers
{
    ...
    public class StudentBedtimeController : Controller
    {
        ...
        public ActionResult ChangDorFunction(string[] stuNumber)
        {
            ...
            {
                for (int i = count; i < stuNumber.Length; i++)
                {
                    ...
                    if (checklist != null)
                    {
                        int n = ChangeDorStudent_Entity.GetList().Where(a => a.DormId == DorId && a.IsDel==false).Max(a => a.BedId);
                        string sqlstr = @"select * from Accdationinformation where Studentnumber='" + stuNumber[i] + "' and EndDate is null";
            ...
            //查看原宿舍的床位号有无大于8的床位
            string maxlistsql = "select * from Accdationinformation where DormId = " + OldDorId + " and BedId >8  and Enddate is null";
            ...
            if (maxlist.Count > 0)
            {
                ...
                for (int h = 0; h < 20; h++)
                {
                    if (number>maxlist.Count) { break; }
                    string Bedisnotsql = "select * from Accdationinformation where DormId = " + OldDorId + " and BedId =" + number + " and Enddate is null";
                    List<Accdationinformation> bedlist = ChangeDorStudent_Entity.GetListBySql<Accdationinformation>(Bedisnotsql);
                    if (bedlist.Count == 0)
                    {
                        maxlist[h].BedId = number;
                        ChangeDorStudent_Entity.UpdateData(maxlist[h]);
                        number++;
                    }
