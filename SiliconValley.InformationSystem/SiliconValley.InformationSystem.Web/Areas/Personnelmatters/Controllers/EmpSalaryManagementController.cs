    ...
    public class EmpSalaryManagementController : Controller
    {
        ...
        public ActionResult EmpSalaryData(int page, int limit, string AppCondition, string ymtime)
        {
            ...
            foreach (var item in newlist)
            {
                ...
                if (attendobj!=null)
                {
                    view.SalaryOne = msrmanage.GetSalaryone(view.baseSalary, view.positionSalary, view.PerformanceSalary, view.netbookSubsidy, view.socialSecuritySubsidy, attendobj.DeserveToRegularDays, attendobj.ToRegularDays, attendobj.LeaveDays, attendobj.AbsenteeismDays,attendobj.NonPersonalLeaveNum);
                }
                #endregion
                //考勤表对象
                ...
                else
                {
                    ...
                    if (view.leavedays > 0)
                    {
                        view.LeaveDeductions = msrmanage.GetLeaveDeductions(view.Id, view.baseSalary, view.positionSalary, view.PerformanceSalary, attendobj.DeserveToRegularDays, view.leavedays);//请假扣款
                    }
                    else
                    {
        ...
        public FileStreamResult MonthlySalaryExport(string time)
        {
            ...
            var data = datalist();
            var attlist = attendance.GetADInfoData().Where(i=> Convert.ToDateTime(i.YearAndMonth).Year == Convert.ToDateTime(time).Year && Convert.ToDateTime(i.YearAndMonth).Month == Convert.ToDateTime(time).Month).ToList();
            //创建工作区
            ...
            void CreateHeader()
            {
                ...
                CreateCell(Header, HeadercellStyle, 24, "合计");
        ...
        //    return View();
        //}
        public List<MySalaryObjView> datalist()
        {
          var  ymtime = FirstTime;
            MonthlySalaryRecordManage msrmanage = new MonthlySalaryRecordManage();//员工月度工资
            EmplSalaryEmbodyManage empsemanage = new EmplSalaryEmbodyManage();//员工工资体系表     
            List<MonthlySalaryRecord> eselist = new List<MonthlySalaryRecord>();
            List<MySalaryObjView> result = new List<MySalaryObjView>();
